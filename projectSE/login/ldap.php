...
if ($username == "" || $password == "") {
	header("Location: login.php?error=กรอกข้อมูลไม่ครบ!!");
	bak;
}
else{
	$info = (user_authen($username, $password, $_POST['username'])); // id
	user_authen("b6020500390", "@Tuck2541", "b6020500390");
	...
	$id=$info[0]["uid"][0];
		...
		$sql = "SELECT * FROM member where kuID='$id'";
	$result=$conn->query($sql);
	$my_row =$result->fetch_assoc();
	$ldap_uid= $my_row['kuID'];

	//เช็คว่ารหัสนี้ มีใน KUWIN ใหม
	if($id!=""){

		//เช็คว่ารหัสนี้มีใน DB ใหม
		...
		//ถ้ายังเพิ่มลงใน DB
			...
			$uid=$info[0]["uid"][0];
			$kuID=$info[0]["idcode"][0];
			$prename=$info[0]["thaiprename"][0];
			$firstname=$info[0]["first-name"][0];
			$lastname=$info[0]["last-name"][0];
			$type=$info[0]["type-person"][0];
			$email=$info[0]["google-mail"][0];
			$sql = "INSERT INTO member (uid,kuID,prename,firstname,lastname,types,email,USERROLES) VALUES ('$uid', '$kuID', '$prename', '$firstname', '$lastname', '$type','$email', 'CLIENT')";
			$result=$conn->query($sql);
			echo "เพิ่มรหัสลงในDatabase เรียบร้อย";
			require("database.close.php");
		}
	}
